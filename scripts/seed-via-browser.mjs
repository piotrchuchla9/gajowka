// Seed Contentful via session token captured from browser (Playwright).
// Workaround dla broken PAT auth — używa Bearer tokenu UI (session-scope, pełne prawa).
...
if (!SPACE) {
  console.error('Missing CONTENTFUL_SPACE_ID in .env');
  process.exit(1);
}

console.log('Otwieram browser. Zaloguj się do Contentful, otwórz space Gajowka, kliknij dowolny entry/Content/Media.');
console.log('Skrypt automatycznie wyłapie token gdy zacznie ruch do api.contentful.com.\n');
...
const tokenPromise = new Promise((resolve) => {
  page.on('request', (req) => {
    const url = req.url();
    if (url.startsWith('https://api.contentful.com/') && url.includes('/spaces/' + SPACE)) {
      const auth = req.headers()['authorization'];
      if (auth && auth.startsWith('Bearer ') && !CMA_TOKEN) {
        CMA_TOKEN = auth.replace(/^Bearer\s+/i, '');
        console.log('✓ Token captured (suffix:', CMA_TOKEN.slice(-6), ')');
        resolve(CMA_TOKEN);
      }
    }
  });
...
await page.goto('https://app.contentful.com/spaces/' + SPACE);
await tokenPromise;

// Verify token works
const verify = await fetch('https://api.contentful.com/spaces/' + SPACE, {
  headers: { Authorization: 'Bearer ' + CMA_TOKEN },
});
if (verify.status !== 200) {
  console.error('Token verify FAILED:', verify.status, await verify.text());
  await browser.close();
  process.exit(1);
}
console.log('✓ Token works, status 200 on space.\n');

// Keep browser open — Contentful refreshes session tokens periodically.
// Will close at the end.

const cf = createClient({ accessToken: CMA_TOKEN }, {
  type: 'plain',
...
// Refresh token from page if rate-limited or expired (call before each batch).
async function refreshToken() {
  // Trigger a navigation to force new auth header
